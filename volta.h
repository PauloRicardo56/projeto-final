...
void cadastrarVolta() {
    int i, resposta, flag = 1, ii, flag2 = 0, j, qtdDados[4];
    char tempData[15];
    struct MelhorVolta voltas[100];
    struct Piloto pilotos[100];
    struct Equipe equipes[100];
    struct Circuito circuitos[100];
    FILE *voltasF, *circuitosF, *equipesF, *pilotosF, *qtdDadosF;
    ...
    for(j=0; j<qtdDados[0]; j++) {
        if(voltas[i].pilotoId == pilotos[j].codigo) {
            break;
        }
    }
    for(ii=0; ii<qtdDados[1]; ii++) {
        printf("@@ %s @@ %s\n", equipes[ii].sigla, pilotos[j].siglaEquipe);
    ...
    // if(qtdDados[3] > 1) {
    //     for(i=0; i<qtdDados[3]; i++) {
    //         if(voltas[i].pilotoId == voltas[qtdDados[3]].pilotoId && voltas[i].voltaData[0] == voltas[qtdDados[3]].voltaData[0]
    //           && voltas[i].voltaData[1] == voltas[qtdDados[3]].voltaData[1] && voltas[i].voltaData[2] == voltas[qtdDados[3]].voltaData[2]) {
    //             voltas[i].tempoVolta[0] = voltas[qtdDados[3]].tempoVolta[0];
    //             voltas[i].tempoVolta[1] = voltas[qtdDados[3]].tempoVolta[1];
    //             voltas[i].tempoVolta[2] = voltas[qtdDados[3]].tempoVolta[2];
    //             flag++;
    //         }
    //     }
    // }
    ...
    voltasF = fopen("voltas", "wb");
    fwrite(voltas, sizeof(struct MelhorVolta), qtdDados[3], voltasF); fclose(voltasF);
